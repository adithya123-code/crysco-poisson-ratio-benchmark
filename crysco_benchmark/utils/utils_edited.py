...
def threshold_sort(matrix, threshold, neighbors, reverse=False, adj=False):
    """
    Keep at most `neighbors` nearest atoms for each atom,
    considering only atoms within `threshold` Angstrom.
    ...
    threshold : float
        Maximum allowed distance in Angstrom.
    neighbors : int
        Maximum number of neighbors per atom.
    reverse : bool
        If False, keep nearest neighbors.
        If True, keep farthest neighbors.
    adj : bool
        If True, also return adjacency information.
    """

    matrix = np.asarray(matrix)

    # Work on a copy so the original distance matrix is not modified.
    distance_matrix_trimmed = np.zeros_like(matrix, dtype=float)

    for i in range(matrix.shape[0]):

        # Candidate atoms within the distance cutoff.
        valid = np.where(
            (matrix[i] > 0) &
            (matrix[i] <= threshold)
        )[0]
        ...
        # Sort candidate atoms by distance.
        if reverse:
            valid = valid[np.argsort(matrix[i, valid])[::-1]]
        else:
            valid = valid[np.argsort(matrix[i, valid])]

        # Keep only the requested number of neighbors.
        selected = valid[:neighbors]

        # Store their actual distances.
        distance_matrix_trimmed[i, selected] = matrix[i, selected]

    if adj:
        adj_list = np.zeros(
            (matrix.shape[0], neighbors),
            dtype=int
        )
        ...
        for i in range(matrix.shape[0]):
            ...
            selected = np.where(
                distance_matrix_trimmed[i] > 0
            )[0]

            n = min(len(selected), neighbors)

            if n > 0:
                adj_list[i, :n] = selected[:n]
                adj_attr[i, :n] = (
                    distance_matrix_trimmed[i, selected[:n]]
                )
